...
class Ad
{
	...
	public static function show($position)
	{
		...
		if($positionObject)
		{
			$adList = self::getAdList($position);
...
<<< OEF
			{$loadJs}
			<div id='AD_{$position}' class='admanage'>
			</div>
			<script type='text/javascript'>
				(new adLoader()).load({$positionJson},{$adJson},"AD_{$position}");
			</script>
OEF;
			echo $htmlOutput;
		}
	...
	private static function display($adData)
	{
		...
		switch($adData['type'])
		{
			...
			case 1:
			{
				...
				(
					'type' => 1,
					'data' => '<img src="'.IUrl::creatUrl("").$adData['content'].'" style="cursor:pointer;'.$size.'" onclick="window.open(\''.IUrl::creatUrl($adData['link']).'\');" />'
				);
			}
			break;
			...
			case 3:
			{
				...
				(
					'type' => 3,
					'data' => '<label style="cursor:pointer;" onclick="window.open(\''.IUrl::creatUrl($adData['link']).'\');">'.$adData['content'].'</label>'
				);
			}
			break;
	...
	public static function getAdList($position)
	{
		...
		return $adDB->query("position_id={$position} AND start_time < '{$now}' AND end_time > '{$now}' ORDER BY `order` ASC ");
	}
}